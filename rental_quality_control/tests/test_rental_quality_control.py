...
class TestRentalQualityControlOca(TestQualityControlOcaBase):
    ...
    def setUpClass(cls):
        ...
        cls.test_rental_prod.rented_product_id.qc_triggers = [
            (
                0,
                0,
                {"trigger": cls.trigger.id, "test": cls.test.id, "timing": "after"},
            )
        ]
    ...
    def test_rental_has_qc_inspection(self):
        ...
        self.assertEqual(
            rental.created_inspections, 1, "Only one inspection must be created"
        )
        inspection = rental.qc_inspections_ids[:1]
        self.assertEqual(inspection.state, "ready")
        self.assertEqual(inspection.qty, rental_out_pick.move_ids.quantity)
        self.assertEqual(
