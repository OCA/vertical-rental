...
class StockPicking(models.Model):
    ...
    def _get_rental_sign_report(self, kind):
        ...
        if not report_id:
            raise UserError(_("No %s report configured in Rental Sign settings.", kind))

        report = self.env["ir.actions.report"].browse(int(report_id))
        if not report or not report.exists():
            raise UserError(_("Configured %s report could not be found.", kind))
        return report
    ...
    def _create_sign_request(self, template, signer_lines, subject_prefix):
        ...
        if not signer_lines:
            raise UserError(_("No signers defined for picking %s.", self.name))
    ...
    def _get_or_create_delivery_sign_request(self):
        ...
        if self.picking_type_id.code != "outgoing" or not self.is_rental_invoice:
            raise UserError(_("This action is only for delivery orders."))
        ...
        if not role_employee:
            raise UserError(_("No Sign role found for Employee."))
        ...
        if role_customer and role_customer in slots_by_role:
            if not self.partner_id:
                raise UserError(
                    _("Picking %s has no customer to sign the document.", self.name)
                )
            signer_lines.append(
                (
                    0,
    ...
    def action_customer_sign_delivery(self):
        ...
        req, role_employee, role_customer = self._get_or_create_delivery_sign_request()

        if not role_customer:
            raise UserError(_("No Sign role found for Customer."))

        signer = req.signer_ids.filtered(
            lambda s, rc=role_customer: s.role_id.id == rc.id
        )[:1]
        if not signer:
            raise UserError(_("No customer signer found on this request."))
    ...
    def action_staff_sign_delivery(self):
        ...
        if not role_employee:
            raise UserError(_("No Sign role found for Staff."))

        signer = req.signer_ids.filtered(
            lambda s, rc=role_employee: s.role_id.id == rc.id
        )[:1]
        if not signer:
            raise UserError(_("No staff signer found on this request."))
    ...
    def action_staff_sign_receipt(self):
        ...
        if self.picking_type_id.code != "incoming" or not self.is_rental_invoice:
            raise UserError(_("This action is only for rental receipt orders."))
        ...
        req = self._create_sign_request(
            template=template,
            signer_lines=signer_lines,
            subject_prefix=_("Return receipt"),
        )
